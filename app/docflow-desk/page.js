...
export default function Home() {
    ...
    () =>
      new DefaultChatTransport({
        ...
        body: () => ({
          ...
          currentDocument: docHtmlRef.current || "",
        }),
      }),
    [],
  );
  ...
  useEffect(() => {
    ...
    for (const part of lastMsg.parts || []) {
      ...
      if (
        ...
        console.log(
          "[Page] Resolving set_document after stream finished:",
          part.toolCallId,
        );
        addToolResult({
          tool: "set_document",
  ...
  const handleElicitSubmit = useCallback(
    async (toolCallId, values) => {
      ...
      console.log(
        "[Page] handleElicitSubmit called. toolCallId:",
        toolCallId,
        "values:",
        values,
      );
      try {
        ...
        console.log(
          "[Page] addToolResult succeeded. Status should auto-resubmit now.",
        );
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      {overlayPhase && (
        <div
          className={`mode-overlay flex justify-center items-center backdrop-blur-sm ${overlayPhase === "in" ? "phase-in" : "phase-out"}`}
        >
          <div className="flex flex-col items-center gap-[1rem]">
            <img src="/logo-white.png" alt="DocFlow AI" width={100} height={100} />
            <h1 className="text-[2rem] font-bold text-white">DocFlow AI</h1>
            <Spinner />
          </div>
        </div>
      )}
